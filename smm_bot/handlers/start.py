...
def _get_menu_kb(db: Database) -> object:
    ...
    return main_menu_kb(official_url=official_url, orders_url=orders_url)
    ...
    if args:
        ...
        if arg.startswith("svc_"):
            ...
            try:
                ...
                pass
        elif arg.startswith("stars_pay_"):
            inv_id = arg[len("stars_pay_"):]
            if inv_id:
                from handlers.invoice_charge import handle_stars_pay_invoice
                await handle_stars_pay_invoice(update, context, inv_id)
                return
