...
class Database:
    ...
    def create_invoice(self, inv_id: str, user_tg_id: int, amount_usd: float) -> str:
        """ينشئ فاتورة جديدة وتنتهي خلال ساعة."""
        with self._conn() as conn:
            conn.execute(
                """INSERT INTO invoices (inv_id, user_tg_id, amount_usd, expires_at)
                   VALUES (?, ?, ?, datetime('now', '+1 hour'))""",
                (inv_id, user_tg_id, amount_usd),
            )
        return inv_id
    ...
    def get_invoice(self, inv_id: str) -> Optional[dict]:
        with self._conn() as conn:
            r = conn.execute(
                "SELECT * FROM invoices WHERE inv_id=?", (inv_id,)
            ).fetchone()
            return dict(r) if r else None

    def mark_invoice_paid(self, inv_id: str, pay_method: str) -> bool:
        """يضع الفاتورة كـ paid ويمنع التكرار عبر عمود credited."""
        with self._conn() as conn:
            r = conn.execute(
                "SELECT credited FROM invoices WHERE inv_id=?", (inv_id,)
            ).fetchone()
            if not r or r["credited"] == 1:
                return False
            conn.execute(
                """UPDATE invoices
                   SET status='paid', pay_method=?, paid_at=datetime('now'), credited=1
                   WHERE inv_id=?""",
                (pay_method, inv_id),
            )
        return True

    def credit_invoice(self, inv_id: str) -> Optional[dict]:
        """يضيف الرصيد للمستخدم مرة واحدة فقط ويعيد بيانات الفاتورة."""
        inv = self.get_invoice(inv_id)
        if not inv:
            return None
        if not self.mark_invoice_paid(inv_id, inv.get("pay_method", "")):
            return None   # سبق إضافته
        self.add_balance(inv["user_tg_id"], inv["amount_usd"])
        return inv

    def get_user_invoices(self, user_tg_id: int, limit: int = 20) -> list:
        with self._conn() as conn:
            rows = conn.execute(
                "SELECT * FROM invoices WHERE user_tg_id=? ORDER BY created_at DESC LIMIT ?",
                (user_tg_id, limit),
            ).fetchall()
