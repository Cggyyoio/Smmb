...
@_guard
async def adm_set_website_url_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    _set_adm_state(context, "waiting_website_url")
    await q.edit_message_text(
        "🌐 أرسل رابط الموقع الإلكتروني:\n(مثال: https://a1smm.store)",
        reply_markup=cancel_kb("adm_channels")
    )
...
async def admin_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    LINK_STATES = {
        ...
        "waiting_website_url":     ("website_url",         "adm_channels"),
