...
async def my_api_key_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await safe_answer(query)
    db: Database = context.bot_data["db"]
    uid = update.effective_user.id

    api_key     = db.get_or_create_api_key(uid)
    website_url = db.get_setting("website_url", "")

    from telegram import InlineKeyboardMarkup, InlineKeyboardButton
    kb_rows = [
        [InlineKeyboardButton("🔄 تجديد المفتاح", callback_data="api_key_regen")],
    ]
    if website_url:
        kb_rows.append([InlineKeyboardButton("🌐 فتح الموقع", url=website_url)])
    ...
    await safe_edit(
        ...
        f"<code>{api_key}</code>\n\n"
        f"📌 استخدمه للدخول للموقع أو الوصول للخدمات برمجياً.\n"
        f"⚠️ لا تشاركه مع أحد — من يملكه يقدر يشحن ويطلب من رصيدك.",
        reply_markup=InlineKeyboardMarkup(kb_rows),
        parse_mode="HTML",
    )


async def api_key_regen_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await safe_answer(query)
    db: Database = context.bot_data["db"]
    uid = update.effective_user.id
    db.regenerate_api_key(uid)
    await my_api_key_callback(update, context)
